...
impl Database {
    pub async fn init() -> Self {
        ...
        let db = client.database("dog-walking");
    ...
    pub async fn create_owner(&self, owner: Owner) -> Result<InsertOneResult, Error> {
        let result = self
        .owner.insert_one(owner).await.ok().expect("Error creating owner");
    ...
    pub async fn create_dog(&self, dog: Dog) -> Result<InsertOneResult, Error> {
        let result = self
        .dog.insert_one(dog).await.ok().expect("Error creating owner");
    ...
    pub async fn create_booking(&self, booking: Booking) -> Result<InsertOneResult, Error> {
        let result = self
        .booking.insert_one(booking).await.ok().expect("Error creating booking");
    ...
    pub async fn get_bookings(&self) -> Result<Vec<FullBooking>, Error> {
        ...
        ]).await.ok().expect("Error getting full bookings");
        ...
        while let Some(result) = results.next().await {
            match result {
                ...
                Err(error) => panic!("Error getting booking: {}", error)
            }
        }
